...
def plot_M_v_J(max_samples = 5000):
    """ Plots the mean magnetization per spin vs. anisotropic strength.
        Data is acquired by calling the M_v_K() function in the C implementation.
    ...
    print("Initializing lattice...")
    _h3d.initialize_lattice()
    num_samples = _h3d.M_v_J(results) # pass results list by ref to be populated by C function
    ...
    with open("sim_results/sim_"+str(SIM_NUM)+".pickle", 'wb') as fp:
        pickle.dump([J_vals, M_vals, M1_vals, M2_vals, M3_vals, M4_vals], fp)
