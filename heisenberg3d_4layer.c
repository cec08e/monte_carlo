...
#define SIM_NUM 385  /* Simulation number */
#define SIM_CONFIG "sim_configs/sim_config.txt"    /* Simulation config file name */

double J_INTRA[4] = {1.0, 1.0, 1.0, 1.0};
double J_INTER[4] = {.025, .1, .1, 0};      /* Inter-layer interaction strength between each pair of layers */
/*double B_EXT = -5;  */                         /* External field strength */
double B_EXT = -.2;
double K[4] = {-.08, -.05, -.05, -.05};      /* Anistropic strength, per layer */
...
int M_v_J(double** results){
    ...
    int sample_counter = 0;
    B_EXT = .23;   /* Critical Switching Field */

    FILE *f = fopen(SIM_CONFIG, "a");
    fprintf(f, "Simulation %d: Size = %d, J_inter = {%f,%f,%f,%f}, K = {%f,%f,%f,%f}, J_intra = {%f,%f,%f,%f}, 10000 steps, T=.15, B=.23 (J sweep)\n ", SIM_NUM, ROWS, J_INTER[0], J_INTER[1],
    J_INTER[2], J_INTER[3], K[0], K[1], K[2], K[3], J_INTRA[0], J_INTRA[1], J_INTRA[2], J_INTRA[3]);
    fclose(f);
    double J_val = .3;
    cool_lattice(.15);
    ...
    while(J_val > .1){
        ...
        results[sample_counter][1] = results[sample_counter][1]/10000.0;
        results[sample_counter][2] = results[sample_counter][2]/10000.0;
        results[sample_counter][3] = results[sample_counter][3]/10000.0;
        results[sample_counter][4] = results[sample_counter][4]/10000.0;
        results[sample_counter][5] = results[sample_counter][5]/10000.0;
        sample_counter += 1;
